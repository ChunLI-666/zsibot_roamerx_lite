...
将 ROS2 标准速度命令 (`geometry_msgs/Twist`) 转换为 ZsiBot SDK 调用的桥接节点。

## 概述

本包提供了 ROS2 导航栈（发布 `cmd_vel` 消息）与 ZsiBot 四足机器人 SDK 之间的桥接功能。节点订阅速度命令并通过 `mc_sdk::zsl_1::HighLevel` 接口将命令转发给机器人。
