...
最新的定位身份—实际规划—执行链进展见 [P2 联动阶段审查](epoch_navigation_review_20260910.md)。下文原数值/模型实验和交接清单保留当时的版本；新一轮不修改其冻结条件。
...
| P2 规控与失效保护 | 前向约束控制器、对齐/停稳；typed 定位—规划—控制—平滑—gate 握手及实际失效恢复 | 带身份的受限运动恢复；平滑动力学与碰撞预测一致性 |
| P3 已有数据闭环 | 新模型控制器矩阵、冻结现场上下文，以及真实 Nav2 节点的身份/失效恢复实验 | 完整全栈轨迹碰撞验收、留出路线及独立参考 |
...
该首轮提交尚未实现跨进程 epoch、定位提交、新全局路径与速度输出之间的握手；收到恢复后的新命令不能证明它属于新规划。随后该工作包已实现 typed 契约，详见本页顶部链接的新一轮报告；原 UInt8 门不能替代它。
...
以上为首次交接时的冻结清单。随后开发的跨进程握手与全栈失效/恢复实验见新一轮报告；下一阶段仍需带身份的受限运动恢复、平滑后的执行轨迹碰撞一致性、独立真值和板端测试。实机尺寸/摆腿资料到位后再冻结可部署 footprint。共享 LM 的回环调用方尚未完整处理优化失败返回值，完整 SLAM 回归和该调用方处理也需单列跟进。总 goal 保持 active；本报告不申请合并，也不表示已经通过整机验收。
